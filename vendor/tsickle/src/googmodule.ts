...
function getGoogNamespaceFromClutzComments(
  ...
  if (hasMultipleProvides) {
    ...
    reportDiagnostic(
      ...
      `referenced JavaScript module ${tsImport} provides multiple namespaces and cannot be imported by path.`);
...
export function getAmbientModuleSymbol(
  ...
  if (!moduleSymbol) {
    ...
    moduleSymbol =
      ...
      tryFindAmbientModuleWithoutAugmentations(t);
  }
  return moduleSymbol;
}

function tryFindAmbientModuleWithoutAugmentations(t: string): ts.Symbol | undefined {
  // throw new Error('tryFindAmbientModuleWithoutAugmentations should not be used');
  return undefined;
}
